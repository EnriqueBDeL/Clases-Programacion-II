...
struct moto* pila_insertar(struct moto *pila, struct moto *pindice, char *modelo, int anio, float precio)
{
    ...
    paux = (struct moto *) malloc(sizeof (struct moto));
    paux->modelo = strdup(modelo); // asignación dinámica de la cadena modelo
    paux->anio = anio;
    paux->precio = precio;
...
void pila_borrar(struct moto *pila, struct moto *pindice)
{
    ...
    paux->psig = pindice->psig;
    free(pindice->modelo); // liberar la memoria asignada para el modelo
    free(pindice);
}
...
void pila_destruir(struct moto *pila)
{
    ...
    while (paux != NULL) {
        pborra = paux;
        paux = paux->psig;
        free(pborra->modelo); // liberar la memoria asignada para el modelo
        free(pborra);
    }
}
